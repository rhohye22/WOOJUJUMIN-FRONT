...
function PartycardBtn() {
  ...
  useEffect(() => {
    const partypost = async () => {
      await axios
        .get("http://118.67.132.98:3000/mainPartypost", { params: {} })
        .then(function(res) {
          ...
          return (
            <Link to={`partybbsdetail/${dto.partySeq}`}>
              <div key={index} className="partycard">
                <img alt="recommemd" src={dto.imageurl} width="130" height="130" style={{ borderRadius: "30px" }}></img>
                <div className="partycontent">
                  ...
                  {dto.tag === 6 && <p style={{ backgroundColor: "#3676d4", color: "white" }}>게임</p>}
                  {dto.tag === 7 && <p style={{ backgroundColor: "#1f1fe2", color: "white" }}>음식</p>}
                  {dto.tag === 8 && <p style={{ backgroundColor: "#6e1fe2", color: "white" }}>우주주민과 함께</p>}
                  {dto.tag === 9 && <p style={{ backgroundColor: "#d193e7", color: "white" }}>우주주민과 탐사</p>}
                  {dto.tag === 10 && <p style={{ backgroundColor: "#585154", color: "white" }}>잡담</p>}
                  <p className="cardTitle">{dot14(dto.title)}</p>
                  <p>
                    <i>
                      {dto.partytype === 1 && "관람 "}
                      {dto.partytype === 2 && "팀원 "}· 모임날짜 {dto.mdate.substring(0, 16)}
                    </i>
                  </p>
